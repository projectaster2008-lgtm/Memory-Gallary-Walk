...
import { getQuickRoamingRemark } from '../utils/narratives';
...
export default function GalleryGlobe({
  ...
  // Outside Casual Remarks & Floating Thoughts
  const [casualThought, setCasualThought] = useState<{ text: string; location: string; x: number; y: number } | null>(null);
  const thoughtCacheRef = useRef<Record<string, string>>({});

  const fetchCasualThought = (targetMem: MemoryItem, step: number) => {
    if (!targetMem) return;
    const cacheKey = targetMem.id;

    // Randomize slight floating position across viewport
    const randomX = Math.floor(Math.random() * 30) - 15; // -15px to +15px
    const randomY = Math.floor(Math.random() * 20) - 10;

    // Fast local remark lookup with rich variations
    const remark = getQuickRoamingRemark(targetMem);
    thoughtCacheRef.current[cacheKey] = remark;
    setCasualThought({ text: remark, location: targetMem.location, x: randomX, y: randomY });
  };
  ...
  // Pick a random memory index, prioritizing unvisited memories first
  const getRandomNextIndex = useCallback(
    (current: number, total: number, recent: number[], visited: number[] = []) => {
      if (total <= 1) return 0;
      const visitedSet = new Set(visited);

      // 1. First priority: Pick randomly from unvisited memories if any remain
      const unvisited: number[] = [];
      for (let i = 0; i < total; i++) {
        if (!visitedSet.has(i) && i !== current) {
          unvisited.push(i);
        }
      }
      ...
      // 2. All visited: Pick candidate avoiding the most recent 3 items
  ...
  const stepToNextRandom = useCallback(() => {
    ...
    if (historyPointer < walkHistory.length - 1) {
      ...
      fetchCasualThought(activeMemoryList[nextIndex], nextPtr + 1);
    } else {
      const nextRandom = getRandomNextIndex(
        walkIndex,
        activeMemoryList.length,
        walkHistory,
        visitedIndices
      );
      // Keep rich history without arbitrary 15-item slicing cap
      const newHistory = [...walkHistory.slice(-200), nextRandom];
      setWalkHistory(newHistory);
      setHistoryPointer(newHistory.length - 1);
      setWalkIndex(nextRandom);
      setVisitedIndices((prev) => (prev.includes(nextRandom) ? prev : [...prev, nextRandom]));
      triggerGsapCameraTransition(nextRandom);
      globalPlaylist.playCardChime(nextRandom);
      fetchCasualThought(activeMemoryList[nextRandom], newHistory.length);
    }
  }, [
    activeMemoryList,
    historyPointer,
  ...
  const stepToPrevious = useCallback(() => {
    ...
    if (historyPointer > 0) {
      ...
      fetchCasualThought(activeMemoryList[prevIndex], prevPtr + 1);
    }
  }, [
    activeMemoryList,
    historyPointer,
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-6 left-0 right-0 flex flex-col items-center gap-3 pointer-events-none z-30 px-4">
        ...
        <AnimatePresence mode="wait">
          {isWalking && currentWalkingMemory && (
            <motion.div
              ...
              className="bg-neutral-950/90 backdrop-blur-xl border border-white/15 shadow-2xl rounded-2xl p-3 sm:p-4 max-w-md w-full flex flex-col gap-2 pointer-events-auto interactive-control text-white"
              style={{
            ...
            >
              <div className="flex items-center gap-3.5">
                  ...
                  <div className="flex items-center justify-between gap-1.5 mb-0.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        ...
                        style={{ color: currentTrack.palette.secondary }}
                      >
                        Step #{historyPointer + 1}
                      </span>
                    </div>
                    <span className="text-[10px] font-mono text-gray-300 bg-white/10 px-2 py-0.5 rounded-full whitespace-nowrap">
                      {visitedIndices.length}/{activeMemoryList.length} Discovered
                    </span>
                  </div>
                ...
                <button
                  ...
                  className="px-3.5 py-2 bg-white hover:bg-gray-100 text-gray-900 text-xs font-semibold rounded-xl transition-all shadow-md flex items-center gap-1.5 flex-shrink-0 hover:scale-105 active:scale-95 cursor-pointer"
                >
              ...
              <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(
                      100,
                      Math.max(5, (visitedIndices.length / activeMemoryList.length) * 100)
                    )}%`,
        ...
        {/* Casual Wandering Remarks / Floating Speech Bubble in 3D Space */}
        <AnimatePresence>
          {casualThought && (
            <motion.div
              key={casualThought.text}
              initial={{ opacity: 0, scale: 0.85, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0, x: casualThought.x }}
              exit={{ opacity: 0, scale: 0.9, y: -10 }}
              transition={{ type: 'spring', damping: 20, stiffness: 300 }}
              className="absolute top-20 sm:top-24 right-4 sm:right-8 max-w-xs sm:max-w-sm pointer-events-auto z-20"
            >
              <div className="bg-neutral-900/90 backdrop-blur-md text-white text-xs px-3.5 py-2.5 rounded-2xl border border-white/15 shadow-2xl relative flex items-start gap-2.5 group">
                ...
                <div className="space-y-1">
                  ...
                  <span className="text-[10px] text-emerald-400/80 font-mono block">
                    — {casualThought.location || 'Dito'}
                  </span>
                </div>
                {/* Close bubble */}
                <button
                  onClick={() => setCasualThought(null)}
                  className="text-gray-400 hover:text-white transition-colors p-0.5 -mr-1 -mt-1 opacity-60 hover:opacity-100 cursor-pointer"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
          ...
          <button
            onClick={() => {
              ...
              if (nextWalk) {
                ...
                // Pick a random starting destination and reset discovery tracking for new walk
                ...
                setHistoryPointer(0);
                setVisitedIndices([initialRandom]);
