...
export default function GalleryGlobe({
  ...
  const fetchCasualThought = async (targetMem: MemoryItem, step: number) => {
    if (!targetMem) return;
    ...
    // If already generated by AI and cached, display it
    if (thoughtCacheRef.current[cacheKey]) {
      const cached = thoughtCacheRef.current[cacheKey];
      setCasualThought({ text: cached, location: targetMem.location, x: randomX, y: randomY });
      return;
    }

    // Request dynamic thought from AI
    try {
      const res = await fetch('/api/globe-narration', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title: targetMem.title,
          location: targetMem.location,
          description: targetMem.description,
          stepNumber: step,
          totalDiscovered: visitedIndices.length,
          totalMemories: activeMemoryList.length,
        }),
      });
      if (res.ok) {
        const data = await res.json();
        if (data.success && data.narration) {
          thoughtCacheRef.current[cacheKey] = data.narration;
          setCasualThought({ text: data.narration, location: targetMem.location, x: randomX, y: randomY });
        }
      }
    } catch {
      // Ignore network failures to prevent overlapping messages
    }
  };
